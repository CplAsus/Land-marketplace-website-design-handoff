(function () {
  ...
  if (!cfg || !cfg.url || !cfg.publishableKey || !window.supabase || !core) {
    root.innerHTML = '<section class="card login"><h1>เชื่อมต่อระบบไม่ได้</h1><p>โหลดส่วนประกอบระบบไม่ครบ กรุณาตรวจสอบอินเทอร์เน็ตแล้วลองอีกครั้ง</p><button class="btn btn-primary" id="retry-start">ลองอีกครั้ง</button></section>';
    document.getElementById('retry-start').onclick = function() { window.location.reload(); };
    return;
  }
  ...
  function googleMapUrl(v) { return core.googleMapUrl(v); }
  function coordinatesFromMapUrl(v) {
    return core.coordinatesFromMapUrl(v);
  }
  ...
  function loginView(message) {
    root.innerHTML = '<section class="card login"><h1>เข้าสู่ระบบผู้ดูแล</h1><p class="muted">ใช้บัญชีที่ได้รับสิทธิ์จากทรายทองพัฒนา</p>' +
      ...
      '<form id="login-form"><div class="field"><label for="login-email">อีเมล</label><input id="login-email" name="email" type="email" required autocomplete="username"></div>' +
      '<div class="field" style="margin-top:14px"><label for="login-password">รหัสผ่าน</label><input id="login-password" name="password" type="password" required autocomplete="current-password"></div>' +
  ...
  async function login(e) {
    ...
    try {
      var result = await client.auth.signInWithPassword({email:String(data.get('email')).trim(),password:data.get('password')});
      if (result.error) throw result.error;
      session = result.data.session; await verifyAdmin();
    } catch (error) {
      var previous = document.getElementById('login-error');
      if (previous) previous.remove();
      var message = document.createElement('div'); message.id = 'login-error'; message.className = 'error'; message.setAttribute('role','alert');
      message.textContent = error.code === 'invalid_credentials' ? 'อีเมลหรือรหัสผ่านไม่ถูกต้อง' : 'เชื่อมต่อระบบไม่ได้ กรุณาตรวจสอบอินเทอร์เน็ตและสถานะระบบ แล้วลองอีกครั้ง';
  ...
  function field(label,name,value,type,required,cls){return '<div class="field '+(cls||'')+'"><label for="field-'+name+'">'+label+'</label><input id="field-'+name+'" name="'+name+'" type="'+type+'" value="'+esc(value)+'" '+(type==='number'?'step="any" min="0" ':'')+(name==='price'?'inputmode="numeric" autocomplete="off" ':'')+(required?'required':'')+'></div>';}
  function area(label,name,value,cls){return '<div class="field '+(cls||'')+'"><label for="field-'+name+'">'+label+'</label><textarea id="field-'+name+'" name="'+name+'">'+esc(value)+'</textarea></div>';}
  function selectField(label,name,value,options,cls){return '<div class="field '+(cls||'')+'"><label for="field-'+name+'">'+label+'</label><select id="field-'+name+'" name="'+name+'">'+options.map(function(o){return '<option value="'+esc(o)+'" '+(o===value?'selected':'')+'>'+esc(o)+'</option>';}).join('')+'</select></div>';}
  ...
  async function saveListing(e) {
    ...
    try {
      ...
      var coordValue=String(f.get('coordinates')||'').trim() || coordinatesFromMapUrl(mapUrl);
      var point=core.coordinates(coordValue);
      var cm=point ? ['',point.lat,point.lng] : null;
      ...
      if(coordValue&&!point)throw new Error('พิกัดไม่ถูกต้อง ละติจูดต้องอยู่ระหว่าง -90 ถึง 90 และลองจิจูด -180 ถึง 180');
      if(!Number.isFinite(Number(f.get('rai')))||Number(f.get('rai'))<=0)throw new Error('กรุณากรอกขนาดที่ดินมากกว่า 0');
      if(!String(f.get('title')||'').trim())throw new Error('กรุณากรอกชื่อประกาศ');
      if(f.has('published')&&f.get('status')==='draft')throw new Error('กรุณาเปลี่ยนสถานะจากฉบับร่างก่อนเผยแพร่');
  ...
  client.auth.getSession().then(function (r) { if(r.error)throw r.error; session=r.data.session; return session?verifyAdmin():loginView(); }).catch(function() { loginView('เชื่อมต่อระบบไม่ได้ กรุณาลองใหม่ภายหลัง'); });
})();
